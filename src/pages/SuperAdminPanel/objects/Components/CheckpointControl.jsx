...
const CheckpointControls = ({ onUndo, onRedo, onClear, canUndo, canRedo }) => {
  const map = useMap();
  ...
  useEffect(() => {
    const Control = L.Control.extend({
      onAdd() {
        ...
        div.innerHTML = `
          <button class="ctrl-btn undo" title="Undo checkpoint">↶</button>
          <button class="ctrl-btn redo" title="Redo checkpoint">↷</button>
          <button class="ctrl-btn clear" title="Clear checkpoints">✕</button>
        `;
